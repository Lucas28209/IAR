...
class Formiga():
    def __init__(self, x,y,raio_visao, grid,its, n_dados, alpha,cont, cont_true):
        self.grid = grid
...
class AntProgram():
    def __init__(self, grid, qntd_dados, dados, raio_visao, num, itr, tam,sleep=0):
        ...
        self.cria_formigas(self.num, self.raio_visao, self.grid, self.itr // self.num, self.n_dados)
    ...
    def cria_formigas(self, num, raio_visao, grid, its, n_dados):
        for i in range(num):
            x = np.random.randint(0,self.size-1)
            y = np.random.randint(0,self.size-1)
            alpha = np.random.uniform(0.0,1.0)  
            formiga = Formiga(x,y,raio_visao, grid,its, n_dados, alpha,cont=0,cont_true=0)
...
if __name__ == "__main__":
    ...
    #print(dados.alpha)
    program = AntProgram(grid=50, qntd_dados=dados.qntd_dados, dados=dados.dados_labels, raio_visao=2, num=20, itr=2000*dados.qntd_dados, tam=650,sleep=1)
    program.run()
    #print(grid)
